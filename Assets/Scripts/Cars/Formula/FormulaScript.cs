...
public class FormulaScript : MonoBehaviour {
	...
	void Start () 
	{
		...
		gearText = GameObject.FindGameObjectWithTag("CarGearHUD").GetComponent<Text>();
	...
	void FixedUpdate()
	{
		...
		wheelColliders[0].attachedRigidbody.AddForce(Vector3.down * 500 * (currentSpeed/350));
		wheelColliders[1].attachedRigidbody.AddForce(Vector3.down * 500 * (currentSpeed/350));
		wheelColliders[2].attachedRigidbody.AddForce(Vector3.down * 500 * (currentSpeed/350));
		wheelColliders[3].attachedRigidbody.AddForce(Vector3.down * 500 * (currentSpeed/350));
	}
	...
	void Engine()
	{
		...
		carEngineSFactor = 1.7f + currentSpeed/maxSpeed * 4;
	...
	void Gears()
	{
		...
		case true:
			...
			case 2:
				...
				if(currentSpeed < maxSpeed)
				{
					gearTorque = acceleration * (2800 - (200 * gear));
				}
				else
				{
					gearTorque = 0;
